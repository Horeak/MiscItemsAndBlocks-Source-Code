...
public class ServerTickHandler{
	...
	public void onPlayerTick(EntityPlayer player) {
        ...
        if(player.inventory.getCurrentItem() != null){
            if(MaterialEnergyUtils.GetValue(player.inventory.getCurrentItem()) != null) {
                System.out.println(MaterialEnergyUtils.GetValue(player.inventory.getCurrentItem()).Value);
            }else {
	...
	public void Flight(EntityPlayer player){
		if(player.inventory.armorInventory[2] == null || player.inventory.armorInventory[2].getItem() != ModItems.FlightChestPlate ){
			...
			if(player.onGround == false && player.isInWater() == false && player.capabilities.allowFlying){
